...
# flash_attn imports removed — patched to use torch SDPA + standard rotate_half rotary.
# This makes MDLM importable without flash-attn (which has brittle CUDA-toolchain
# build requirements and was failing on systems with torch built against a newer
# CUDA toolkit than the installed nvcc).
import torch
...
def apply_rotary_pos_emb(qkv, cos, sin):
  # qkv: (b, s, 3, h, d). cos/sin come in shape (1, s, 1, 1, d) from Rotary above.
  # Following the original (which used flash_attn's apply_rotary_emb_qkv_ with
  # `interleaved=False`, the GPT-NeoX convention), we apply standard half-rotation
  # to q and k and pass v through.
  cos_half = cos[0, :, 0, 0, :cos.shape[-1] // 2]   # (s, d/2)
  sin_half = sin[0, :, 0, 0, :sin.shape[-1] // 2]
  cos_full = torch.cat([cos_half, cos_half], dim=-1)[None, :, None, :]  # (1, s, 1, d)
  sin_full = torch.cat([sin_half, sin_half], dim=-1)[None, :, None, :]
...
class DDiTBlock(nn.Module):
  ...
  def forward(self, x, rotary_cos_sin, c, seqlens=None):
    ...
    # Replaced flash_attn_varlen_qkvpacked_func with torch's scaled_dot_product_attention.
    # Since this MDLM variant pads to a fixed seq_len, cu_seqlens just describes
    # equal-length packed sequences — equivalent to regular fixed-length attention
    # done batch-wise. Bidirectional (causal=False).
    # qkv shape here is still (b, s, 3, h, d) — we haven't done the (b s) -> bs flatten yet.
    q = qkv[:, :, 0].transpose(1, 2)  # (b, h, s, d)
    k = qkv[:, :, 1].transpose(1, 2)
    v = qkv[:, :, 2].transpose(1, 2)
    x = F.scaled_dot_product_attention(q, k, v, is_causal=False, dropout_p=0.0)
    x = x.transpose(1, 2).contiguous()  # (b, s, h, d)
