...
| [small-sae-bench](https://github.com/legibleweights/small-sae-bench) | **v0.4.1:** Register-Subtracted TopK SAE wins on CE recovery (+0.3 to **+3.9 pts** over vanilla TopK) across 3 models × 6 (model, layer) configs, with the refined rule: **RS gain is driven by position-0 residual magnitude × prefix-reconstruction-error gap**, not just by TopK's prefix-EV failure. Pythia L22 (peak-magnitude register before the L23 eraser) shows the biggest gain (+3.9 pts) even though TopK's prefix EV is 0.977 (not catastrophic). Honest cost: at Pythia L22 RS pays 4.2pts mid-seq EV — a real Pareto cost. Selection rule for practitioners now in NOTES.md. Zero extra learnable parameters; uses the fixed register from `outlier-position-anatomy`. |
