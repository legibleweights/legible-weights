...
| [small-sae-bench](https://github.com/legibleweights/small-sae-bench) | **v0.4.2:** Register-Subtracted TopK SAE wins on CE recovery in **5 of 7** (model, layer) configs tested across 3 small open transformers (gains 0.3 to **+3.9 pts**), ties once (Pythia L12), and slightly regresses once (**GPT-2 L10, −0.4 pts** — the v0.4.1 magnitude rule doesn't fit here; possibly because GPT-2's eraser is attention-head-mediated). Always gives perfect prefix reconstruction (EV ≥ 0.999 vs TopK's −1.25 to +0.94 range). **Not universally Pareto-better — practitioners should A/B test.** Zero extra learnable parameters; uses the fixed register from `outlier-position-anatomy`. |
