...
| [outlier-position-anatomy](https://github.com/legibleweights/outlier-position-anatomy) | The position-0 attention-sink in small open transformers (Qwen2.5-0.5B, GPT-2 small, Pythia-1.4B) is a load-bearing 3-component write-and-erase register circuit, not noise. MLPs do most of the work (not attention heads). Ablating the writers eliminates the outlier but costs +3.3 nats CE loss on Qwen — the model has learned to use it. Same topology emerges across all three architectures. |
